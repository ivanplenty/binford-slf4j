...
LoggerInterface.prototype.addAppender = function(appender, marker, level, options){
	...
	if(concreteLogger.supportsSlf4j && concreteLogger.addAppender)
	{
		concreteLogger.addAppender(appender, marker, level, options);
	}
	else
	{
...
LoggerInterface.prototype.detachAppender = function(name){
	if(concreteLogger.supportsSlf4j && concreteLogger.detachAppender)
	{
		concreteLogger.detachAppender(name);
	}
	else
	{
		throw "The concrete logger is not compatible with slf4j";
...
LoggerInterface.prototype.detachAndStopAllAppenders = function(){
	if(concreteLogger.supportsSlf4j && concreteLogger.detachAndStopAllAppenders)
	{
		concreteLogger.detachAndStopAllAppenders(name);
	}
	else
	{
		throw "The concrete logger is not compatible with slf4j";
...
LoggerInterface.prototype.setLevel = function(newLevel){
	...
	if(!concreteLogger)
	{
		throw "No concrete logger available";
	}
